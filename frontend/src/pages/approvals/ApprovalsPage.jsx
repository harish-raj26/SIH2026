...
export function ApprovalsPage() {
  ...
  const handleDiscover = async () => {
    ...
    try {
      const data = await approvalService.discoverApprovals(
        activeBusiness.id
      );
      ...
      showSuccess(
        `Discovery complete. ${newApprovalCount} new approval(s) found. ${totalApprovalCount} total approval(s) available.`,
        'Discovery Complete'
      );
      ...
      showError(
        err.message || 'Approval discovery failed',
        'Discovery Error'
      );
    } finally {
  ...
  const loadRoadmap = useCallback(async (businessId) => {
    ...
    try {
      const data = await approvalService.getRoadmap(
        businessId
      );
      ...
      console.warn(
        'Could not fetch statutory roadmap:',
        err.message
      );
    } finally {
  ...
  const loadAiRoadmap = async () => {
    ...
    try {
      ...
      showSuccess(
        'AI Compliance Executive Roadmap generated successfully',
        'Gemini AI'
      );
    } catch (err) {
      showError(
        err.message || 'AI Roadmap generation failed',
        'AI Roadmap Error'
      );
    } finally {
  ...
  const displayedApprovals =
    approvalsData?.approvals?.length > 0
      ...
      : roadmapData?.roadmap?.map((r) => ({
          ...
          reason:
            `Statutory requirement derived for ${activeBusiness.industry}`,
          regulatory_evidence:
            r.regulatory_evidence || [],
        })) || [];
          ...
          {discovering ? (
            <div className="p-8 text-center space-y-3 bg-white rounded-xl border border-[#E2E8E7]">
              ...
              <p className="text-xs text-[#66757A] max-w-md mx-auto">
                Evaluating industry criteria, workforce thresholds,
                and safety rules to derive required permits.
              </p>
            </div>
                      ...
                      <CardContent className="space-y-3 pt-2 text-xs">
                        {appr.reason && (
                          <div className="p-3 rounded-lg bg-[#F8FAF9] border border-[#E2E8E7] text-[#172126]">
                            ...
                            <span className="text-[#66757A]">
                              {appr.reason}
                            </span>
                          </div>
                        )}

                        <RegulatoryEvidenceCard
                          evidence={
                            appr.regulatory_evidence || []
                          }
                        />
                      </CardContent>
                    </div>
          ...
          {loadingRoadmap ? (
            ...
            <Card>
              <CardHeader className="pb-4">
                <CardTitle>
                  Statutory Sequence & Priorities
                </CardTitle>

                <CardDescription>
                  Follow these sequential departmental clearances
                  for statutory operational clearance.
                </CardDescription>
              </CardHeader>
                      ...
                      <div className="flex items-start gap-3.5">
                        ...
                        <div className="space-y-0.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span
                              className={`text-[10px] font-medium px-2 py-0.5 rounded border ${pStyle.bg} ${pStyle.text} ${pStyle.border}`}
                            >
        ...
        <div className="space-y-4">
          {!aiRoadmapData ? (
            <div className="p-8 text-center space-y-3 bg-white rounded-xl border border-[#E2E8E7]">
              ...
              <p className="text-xs text-[#66757A] max-w-md mx-auto">
                Synthesize statutory evidence into concrete
                preparation steps, inspection alerts, and
              ...
              <Card className="bg-[#E6F2F2] border-[#BFE0DF]">
                <CardHeader>
                  ...
                  <CardDescription className="text-[#004F4D] text-xs leading-relaxed">
                    {aiRoadmapData.summary ||
                      'AI Compliance Roadmap generated from statutory evidence.'}
                  </CardDescription>
                </CardHeader>
              </Card>
